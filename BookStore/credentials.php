...
$username="";$password="";
if($id != ""){
	$sql="SELECT * FROM credentials WHERE customerID = '$id';";
...
<form action="modifyindex.php?id=&form=">
			<div style="margin-top: 25; text-align: center; background: #E2BE9E;
				...
				<input name="form" value="credentials" hidden />
				<br/>			
				<br/>
				
				Username: <input type="text" id="usern" value="<?php echo $username;?>" <?php echo $read;?> />
				Password: <input type="text" id="passw" value="<?php echo $password;?>" <?php echo $read;?> /><br/>
				<br />
...
<script>
	...
	document.getElementById("usern").oninput = function(){
		document.getElementById("usern").name="username";
		document.getElementById("mod").disabled=false;
	}
	document.getElementById("passw").oninput = function(){
		document.getElementById("passw").name="password";
		document.getElementById("mod").disabled=false;
	}
</script>		
<style type="text/css">
